...
Then look at your `edit.html.erb` file. Remove the entirety of the form_for and add the line which renders the partial.
...
```ruby
expect(page).to have_content("Article 'Your Updated Title' Updated!")
```
...
We need to add the flash messages to our view templates. We stored our message in the flash object, but we didn't print it to the page. The `update` method redirects to the `show`, so we _could_ just add the display to our show template.
